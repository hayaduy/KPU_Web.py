...
# --- 1. KONFIGURASI SEMUA URL ENDPOINT (VERSI TERKINI) ---
...
def pop_menu_mandiri(user):
    ...
    if st.button("KLIK UNTUK ABSEN (HADIR)", use_container_width=True, type="primary"):
        ...
        with st.spinner("Menghantar data absen..."):
            try:
                res = requests.post(target_url, json=payload_absen, timeout=10)
                if "Success" in res.text:
                    st.success(f"✅ Absen {status_peg} Berjaya!")
                    st.balloons()
                else:
    ...
    if st.button("HANTAR LAPORAN KINERJA", use_container_width=True):
        if uraian.strip():
            # Data dihantar ke Kolom B, C, D, E, F (No diuruskan oleh Apps Script)
            payload_lapkin = {
                "nama": user['nama'], 
                "nip": nip, 
                "jabatan": jabatan, 
            ...
            with st.spinner("Menghantar laporan ke Google Sheets..."):
                try:
                    ...
                    if "Success" in res.text:
                        st.success("✅ Laporan Kinerja Berjaya Dihantar!")
                    else:
